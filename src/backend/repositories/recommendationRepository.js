...
class RecommendationRepository {
   ...
   async likeArticle(userId, articleId, category) {
      ...
      await pool.query(`UPDATE articles SET likes = COALESCE(likes, 0) + 1 WHERE id = $1`, [articleId]);
   }
   ...
   async unlikeArticle(userId, articleId) {
      const result = await pool.query(`DELETE FROM user_likes WHERE user_id = $1 AND article_id = $2 RETURNING id`, [
         userId,
         articleId,
      ]);
      if (result.rows.length > 0) {
         await pool.query(`UPDATE articles SET likes = GREATEST(COALESCE(likes, 0) - 1, 0) WHERE id = $1`, [articleId]);
      }
   ...
   async isLiked(userId, articleId) {
      const result = await pool.query(`SELECT id FROM user_likes WHERE user_id = $1 AND article_id = $2`, [
         userId,
         articleId,
      ]);
      return result.rows.length > 0;
   ...
   async getUserLikes(userId) {
      const result = await pool.query(`SELECT article_id FROM user_likes WHERE user_id = $1`, [userId]);
   ...
   async getRecommendations(userId, limit = 12) {
    ...
    `,
      ...
      if (recommendations.length < limit) {
         ...
         const placeholders =
            allExclude.length > 0 ? `AND id NOT IN (${allExclude.map((_, i) => `$${i + 2}`).join(',')})` : '';
